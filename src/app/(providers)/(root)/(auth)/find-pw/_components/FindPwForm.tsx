...
import 'react-toastify/dist/ReactToastify.css'; // 기본 toastify CSS 불러오기
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (userError || !userData) {
        toast.error(
          <div className="toast-message">
            <span>가입되지 않은 이메일입니다. 회원가입 먼저 진행해주세요.</span>
            <button
      ...
      if (error) {
        ...
        toast.success(
          <div className="toast-message">
            <span>비밀번호 재설정 링크가 이메일로 전송되었습니다.</span>
          </div>,
          {
            className: 'custom-toast',
            icon: false,
      ...
      toast.error(
        <div className="toast-message">
          <span>서버와의 통신 중 오류가 발생했습니다.</span>
        </div>,
        {
          className: 'custom-toast',
          icon: false,
